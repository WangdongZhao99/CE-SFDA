...
class AbstractTrainer(object):
   ...
   This class contain the main training functions for our AdAtime
    ...
    def __init__(self, args):
        ...
        self.is_weighted = args.is_weighted
    ...
    def train_model(self, src_id, trg_id):
        # Get the algorithm and the backbone network
        algorithm_class = get_algorithm_class(self.da_method)
        backbone_fe = get_backbone_class(self.backbone)

        # Initilaize the algorithm
        self.algorithm = algorithm_class(backbone_fe, self.dataset_configs, self.hparams, self.device, self.is_weighted)
    ...
    def evaluate(self, test_loader):
        ...
        classifier_aad = self.algorithm.classifier_aad.to(self.device)
        ...
        total_loss, preds_list, labels_list, aad_preds_list = [], [], [], []
        total_features, old_logits = [], []
        ...
        with torch.no_grad():
            for data, labels, _ in test_loader:
                ...
                predictions_aad = classifier_aad(features)
                if self.is_weighted and self.test_method=='enesmble':
                    softmax1 = F.softmax(predictions, dim=1)
                    softmax2 = F.softmax(predictions_aad, dim=1)
                    entropy1 = -torch.sum(softmax1 * torch.log2(softmax1 + 1e-5), dim=1)
                    entropy2 = -torch.sum(softmax2 * torch.log2(softmax2 + 1e-5), dim=1)
                    weight = torch.exp(-entropy1) / (torch.exp(-entropy1) + torch.exp(-entropy2))
                    predictions = weight[:, None] * predictions + (1 - weight)[:, None] * predictions_aad

                # compute loss
                loss = F.cross_entropy(predictions, labels)
                total_loss.append(loss.item())
                pred = predictions.detach()  # .argmax(dim=1)  # get the index of the max log-probability
                pred_aad = predictions_aad.detach()

                # append predictions and labels
                aad_preds_list.append(pred_aad)
        ...
        self.aad_preds = torch.cat(aad_preds_list)
        self.full_labels = torch.cat((labels_list))
        self.total_features = torch.cat(total_features)    
        if self.test_method == 'learnable':
            self.full_preds = self.aad_preds    
        if self.test_method == 'ensemble':   
            if not self.is_weighted:
                max_logits = []
                for i in range(self.aad_preds.shape[0]):
                    v1 = torch.nn.Softmax(dim=-1)(self.aad_preds[i]).max()
                    v2 = torch.nn.Softmax(dim=-1)(self.full_preds[i]).max()
                    if v1 > v2:
                        max_logits.append(self.aad_preds[i])
                    else:
                        max_logits.append(self.full_preds[i])
                max_logits = torch.cat([i.unsqueeze(0) for i in max_logits], dim=0)
                self.full_preds = max_logits

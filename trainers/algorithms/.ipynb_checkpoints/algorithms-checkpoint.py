...
class OURS(Algorithm):
    ...
    def __init__(self, backbone, configs, hparams, device):
        super(OURS, self).__init__(configs)
        self.feature_extractor = backbone(configs)
        self.classifier = classifier(configs)
    ...
    def pretrain(self, src_dataloader, avg_meter, logger):
        ...
        for epoch in range(1, self.hparams["num_epochs"] + 1):
            ...
            for key, val in avg_meter.items():
                logger.debug(f'{key}\t: {val.avg:2.4f}')
            logger.debug(f'-------------------------------------')
    # 使用shot和AaD进行模型融合，利用这两个算法各自的优点提升性能
    # 舍弃了shot聚类生成伪标签的过程，舍弃了AaD推走的损失（类别数少，在此场景中不合理）
    # shot分类器是固定的（源域预训练好的），AaD分类器（源域预训练好的）是可学习的
    def update(self, trg_dataloader, avg_meter, logger):#ours
        best_src_risk = float('inf')
        best_model = self.network.state_dict()
        last_model = self.network.state_dict()
        classifier_state_dict = self.classifier.state_dict()
        self.classifier_aad.load_state_dict(classifier_state_dict)#AaD分类器初始化
        for k, v in self.classifier.named_parameters():
            v.requires_grad = False
        num_samples = len(trg_dataloader.dataset)
        fea_bank = torch.randn(num_samples, self.configs.final_out_channels * self.configs.features_len)    #AaD算法存储的所有目标域数据的归一化特征
        score_bank = torch.randn(num_samples, self.configs.num_classes).cuda()                              #AaD算法存储的所有目标域数据的概率
        temp = True   #True：在开始时遍历完所有数据，将特征和概率存入bank     False：当前iter的特征和概率更新bank中对应位置
        for epoch in range(1, self.hparams["num_epochs"] + 1):
            for step, (trg_x, y, trg_idx) in enumerate(trg_dataloader):
                ...
                with torch.no_grad():#计算当前batch中特征和bank中特征的相似性，对每个样本选择出topk个样本
                ...
                loss_aad = torch.mean((F.kl_div(softmax_out_un, score_near, reduction='none').sum(-1)).sum(1))#AaD拉近的损失，拉近样本与检索出topk样本的KL散度
                with torch.no_grad():
                    ...
                    weighted_logits = weight[:, None] * trg_pred + (1 - weight)[:, None] * trg_pred_aad#对两个分类器的概率根据熵的大小加权
                    ...
                    # w = w / max_entropy
                    # w = torch.exp(-w)#计算加权logits的熵，对每个样本在后续计算交叉熵损失时赋予不同的权重

                entropy_loss = self.hparams['ent_loss_wt'] * torch.mean(EntropyLoss(softmax_out))
                entropy_loss -= self.hparams['im'] * torch.sum(
                    -softmax_out.mean(dim=0) * torch.log(softmax_out.mean(dim=0) + 1e-5))
                _, pseudo_label = weighted_softmax.max(dim=1)#加权logits矫正的伪标签
                target_loss = self.cross_entropy(trg_pred.squeeze(), pseudo_label.long())#对shot分类器的交叉熵损失

...
class projector(nn.Module):
    def __init__(self, configs):
        ...
        self.projector = nn.Sequential(*[
            nn.Linear(model_output_dim * configs.final_out_channels, 64),
            nn.BatchNorm1d(64),
            nn.ReLU(),
            # # # nn.Dropout(),
            nn.Linear(64,64)
...
class classifier_aad(nn.Module):
    def __init__(self, configs):
        super(classifier_aad, self).__init__()

        model_output_dim = configs.features_len
        self.logits = nn.Linear(model_output_dim * configs.final_out_channels, configs.num_classes*2)
